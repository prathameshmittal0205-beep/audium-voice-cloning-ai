...
This bridges the Vercel cloud infrastructure directly to your local GPU.
...
The Python application (`app.py`) includes auto-registration logic that runs every 60 seconds. It will attempt to detect the running Ngrok tunnel and send a POST request to your deployed Vercel backend to register itself.
...
```bash
curl -X POST https://audium.vercel.app/api/internal/register-worker \
...
*Note: `<YOUR-WORKER-SECRET>` must match the `WORKER_SECRET` you set in the Vercel environment variables.*
...
2. The Vercel backend encounters a cold start (due to Vercel Functions being serverless and stateless).
